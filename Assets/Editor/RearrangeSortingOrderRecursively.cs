...
public class RearrangeSortingOrderRecursively : EditorWindow {
	...
	private static void FindInGO (GameObject g, int index) {
		...
		if (sr) {
			...
			g.transform.localPosition = new Vector3 (g.transform.localPosition.x, order[index] * yStep, g.transform.localPosition.z);
			g.transform.parent.localPosition = new Vector3 (g.transform.parent.localPosition.x, 0.0f, g.transform.parent.localPosition.z);
			order[index]++;
			replacing_count++;
